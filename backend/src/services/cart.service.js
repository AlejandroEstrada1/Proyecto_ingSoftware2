...
/**
 * Vacia todas las lineas del carrito del usuario.
 * @param {number} userId
 */
...
/**
 * Checkout simulado: valida stock, crea pedido, aprueba pago simulado,
 * descuenta stock y vacia el carrito en una transaccion.
 * @param {number} userId
 * @param {{ cardNumber: string }} payment
 */
function checkout(userId, payment) {
  const db = getDb()
  ...
  const paymentResult = approveSimulatedPayment(payment)
  const orderResult = db.transaction(() => {
    ...
    const orderInfo = db
      .prepare(
        `INSERT INTO orders
         (user_id, total, payment_status, payment_reference, card_last4)
         VALUES (?, ?, ?, ?, ?)`
      )
      .run(
        userId,
    ...
    return { orderId, total: cart.subtotal, payment: paymentResult }
  })()

  return orderResult
}

function approveSimulatedPayment(payment) {
  const cardLast4 = payment.cardNumber.slice(-4)
  const authorizationCode = `SIM-${Date.now().toString(36).toUpperCase()}`
  return {
    status: "approved",
    authorizationCode,
    cardLast4,
